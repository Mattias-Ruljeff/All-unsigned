...
const AlbumListDetails = ({ album, albumType, removeAlbumFromList, bandId }) => {
    ...
    if(albumType) {
        albumType.forEach(albumType => {
            if(albumType.id === album.album_type_id) albumTypeHere = albumType.type
        });
    }
                ...
                <table>
                    ...
                    </tr>
                    {
                        songs !== null ? songs.map((song, index) => {
                            return (<SongDetails key={song.id} index={index} song={song} removeAlbumFromList={removeAlbumFromList} />)
                        }) : ""
                    }
                    </tbody>
                </table>
...
// {albumType.map(type => {if(type.id === album.type) return type} )}